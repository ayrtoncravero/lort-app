<template>
  <v-container class="py-10" style="max-width: 1100px">
    <div class="mb-8">
      <h1 class="font-heading text-h3 font-weight-bold mb-1" style="color: #f1ede0">Gods</h1>
      <p class="text-body-1 text-muted">Eru, the Valar and the Maiar of Middle-earth.</p>
    </div>
    ...
    <template v-else-if="gods.length">
      <v-row>
        <v-col v-for="g in gods" :key="g.id" cols="12" sm="6" md="4">
          <GodCard :god="g" />
        </v-col>
      </v-row>
    ...
    <EmptyState v-else message="No gods found. Try changing your search or filters." />
  </v-container>
</template>
...
async function load() {
  ...
  error.value = ''
  try {
    const result = await ainurApi.getAll({
      page: page.value,
      limit,
      name: name.value || undefined,
      type: type.value === ALL_TYPES ? undefined : type.value
    })
    gods.value = result.data
...
async function loadFilterOptions() {
  const result = await ainurApi.getAll({ limit: 100 })
  typeOptions.value = [ALL_TYPES, ...new Set(result.data.map((g) => g.type).filter(Boolean))]
}
