<template>
  <v-container class="py-10" style="max-width: 1100px">
    <div class="mb-8">
      <h1 class="font-heading text-h3 font-weight-bold mb-1" style="color: #f1ede0">Races</h1>
      <p class="text-body-1 text-muted">Explore the peoples of Middle-earth.</p>
    </div>
    ...
    <template v-else-if="races.length">
      <v-row>
        <v-col v-for="r in races" :key="r.id" cols="12" sm="6" md="4">
          <RaceCard :race="r" />
        </v-col>
      </v-row>
    ...
    <EmptyState v-else message="No races found. Try changing your search or filters." />
  </v-container>
</template>
...
async function load() {
  ...
  error.value = ''
  try {
    const result = await racesApi.getAll({
      page: page.value,
      limit,
...
async function loadFilterOptions() {
  const result = await racesApi.getAll({ limit: 100 })
  typeOptions.value = [ALL_TYPES, ...new Set(result.data.map((r) => r.type).filter(Boolean))]
}
